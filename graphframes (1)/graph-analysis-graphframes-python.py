...
bike_stations = spark.sql("SELECT * FROM station_csv")
trip_data = spark.sql("SELECT * FROM trip_csv")

# COMMAND ----------

display(bike_stations)
...
print(f"Total Number of Trips in Original Data: {trip_data.count()}")
...
display(out_deg.orderBy(F.desc("outDegree")).limit(5))
...
display(degree_ratio.orderBy(F.desc("degreeRatio")).limit(10))
...
display(degree_ratio.orderBy(F.asc("degreeRatio")).limit(10))
